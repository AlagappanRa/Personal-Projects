...
def valid_move(game, start, end):
    #invalid if no match are removed
    #invalid if matches are removed from 2 diff piles (unless one is empty)
    #end is always >start
    if end< game[0][0]:
        return False
    if start > game[-1][-1]:
        return False
    #if none of the values between start and end are in the game state.
    flag = False
...
def k_nearest_listings(fname, latitude, longitude, k):
    listings = read_csv(fname)
    
    closest_dist = 9999999.99
    closest = 0
    res = []
    for i in range(len(listings)):
        if i == 0:
            continue
        if get_distance(float(listings[i][4]), float(listings[i][5]),latitude,longitude)<= closest_dist:
            closest_dist = get_distance(float(listings[i][4]), float(listings[i][5]),latitude, longitude)
            closest = i
    #finding the top k
    for i in range(k):
        res.append(get_distance(float(listings[closest][4]), float(listings[closest][5]),latitude, longitude))
        closest+=1
    #print(res)
    #addding the extra values which equal k:
    for i in range(len(listings)):
        if i == 0:
            continue
        if get_distance(float(listings[i][4]), float(listings[i][5]),latitude, longitude)== res[-1]:
            res.append(get_distance(latitude, longitude, float(listings[i][4]), float(listings[i][5])))
    return res
### Your answer here.
# Q2B      
def neighbourhood_price_per_region(fname, region):
    listings = read_csv(fname)
    res = {}
    listings_in_region = list(filter(lambda x: x[2] == region and int(x[7])>=1, listings[1:]))
    areas_in_region = list(map(lambda x: x[3], listings_in_region))
    #finding all the areas in the east region:
    unique_areas = []
    for i in areas_in_region:
        if i not in unique_areas:
            unique_areas.append(i)
    for i in unique_areas:
        price = 0
        count = 0
        for j in listings_in_region:
            if j[3]==i:
                price+= float(j[6])
                count+=1
        res[i] = round(price/count,2)
...
class Doll:
    ...
    def encase(self,other):
        if self.encasing:
            return f'{self.name} already contains {self.encasing[0].name}'
        elif self.encased:
            return f'{self.name} is currently encased in {self.encased[0].name}'
        elif other.encased:
            return f'{other.name} is currently encased in {other.encased[0].name}'
        else:
            other.encased.append(self)
            self.encasing.append(other)
            return f'{self.name} encases {self.encasing[0].name}'
    def release(self):
        if self.encased:
            return f'{self.name} is currently encased in {self.encased[0].name}'
        elif self.encasing:
            return f'{self.name} already contains {self.encasing[0].name}'
        elif not self.encasing:
            return f'{self.name} does not contain any dolls'
        else:
            return f'{self.name} releases {self.encasing[0].name}'
    def get_name(self):
        return self.name
    def get_mother(self):
        if self.encased:
            return self.encased[0]
        else:
            return None
    def get_daughter(self):
        if self.encasing:
            return self.encasing[0]
        else:
            return None
    def series(self):
        #adding towards mother
        ans = (self.name,)
        curr = self
        curr2 = self
        while curr.get_mother():
            ans = (curr.get_mother().name,)+ ans
            curr = curr.get_mother()
        while curr2.get_daughter():
            ans = ans+ (curr2.get_daughter().name,)
            curr2 = curr2.get_daughter

        return ans

        #adding towards daughter
        
    def deeply_contains(self,other):
        series = self.doll_series()
        for i in range(len(series)):
            if series[i]== self:
                if other in series[i:]:
                    return True
        return False
